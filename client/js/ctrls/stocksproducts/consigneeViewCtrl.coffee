...
clickDeleteConsignee = (data)->
  require ['Auth', '$', 'jAlert'], (Auth)->
    jConfirm '确认删除？', '警告', (delete_)->
      ...
      $.getJSON(Auth.apiHost + 'mywms2/goods/consignee/delete', {consigneeId:data.id}
        ,(data)->
          if data.status == 0
            jAlert '删除成功！', '提示'
          else
      ...
      datagrid = $('#consigneeList').datagrid({
        ...
        col:[{
            ...
            render: (data)->
              ...
              "<a href='javascript:clickDeleteConsignee(#{JSON.stringify(data.row)})' class='table-actions-button ic-table-delete'></a>"
          },{
            field: 'name'
            title: '供应商名称'
          }, {
